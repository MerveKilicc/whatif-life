...
export default function FinalPage() {
  ...
  const { avatarData, birthData, history, stats, reset, finalLetter, setFinalLetter } = useSimulationStore();
  const [loading, setLoading] = useState(true);
  const [isClient, setIsClient] = useState(false);

  // Lock to prevent React Strict Mode duplicate fetch
  ...
  useEffect(() => {
    ...
    // Attempt to load cached letter first
    if (finalLetter) {
      setLoading(false);
      return;
    }

    // Prevent concurrent duplicate fetch during Strict Mode multi-mount
    ...
    async function fetchLetter() {
      ...
      if (result.success && result.data) {
        setFinalLetter(result.data);
      } else {
        setFinalLetter("The stars are silent... (Error generating letter)");
      }
      setLoading(false);
    }

    fetchLetter();
  }, [avatarData, birthData, history, stats, router, finalLetter, setFinalLetter]);
  ...
  return (
    <div className="min-h-screen bg-[#16213e] text-white flex flex-col items-center justify-center p-6 relative overflow-hidden">
      {/* Background Image */}
      <img
        src="https://image.pollinations.ai/prompt/vintage%20mysterious%20library%20desk%20with%20celestial%20maps%20and%20candles%20dark%20mood%20cinematic?width=1080&height=1920&nologo=true&seed=42"
