...
export default function SimulationPage() {
    ...
    useEffect(() => {
        if (isClient && !currentChapter) {
            router.push(`/${locale}/input`);
        }
    }, [currentChapter, router, locale, isClient]);

    // Scroll to top when chapter changes to ensure user reads from the beginning of the new chapter
    useEffect(() => {
    ...
    return (
        <div className="min-h-screen bg-[#16213e] text-white flex flex-col items-center relative overflow-x-hidden overflow-y-auto">
            ...
            {/* Top Bar - Stats */}
            <div className="fixed top-0 left-0 right-0 z-50 bg-[#16213e]/90 backdrop-blur-md border-b border-gray-800 p-3 sm:p-4 flex justify-center gap-2 sm:gap-4 shadow-lg">
            ...
            {/* Main Content Area */}
            <div className="w-full max-w-2xl px-6 pt-28 pb-48 flex flex-col gap-8 z-10">
                ...
                {currentChapter && (
                    ...
                    >
                        <div className="flex flex-col sm:flex-row sm:items-baseline justify-between text-[#ffd700] font-serif border-b border-gray-700 pb-2 mb-2">
                            <h1 className="text-xl sm:text-2xl">{currentChapter.title}</h1>
                            <span className="text-xs sm:text-sm opacity-80 mt-1 sm:mt-0">{currentChapter.period} (Yaş: {currentChapter.age})</span>
                        </div>
            ...
            {/* Bottom Action Area */}
            <div className="fixed bottom-0 left-0 right-0 p-4 sm:p-6 bg-[#16213e] border-t border-gray-800 shadow-[0_-20px_40px_rgba(15,21,38,0.9)] z-40 flex flex-col items-center">
                {isLoading ? (
